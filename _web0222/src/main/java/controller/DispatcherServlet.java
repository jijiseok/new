...
//
//			// 포워딩 작업
//			request.setAttribute("list", list); // 객체받은걸 request에 집어 넣는것
			...
			BoardDao dao = BoardDao.getInstance();
			Board board = dao.selectOne(num, true); // true는 히트수 늘려주는것
			...
			// 포워딩 작업
			request.setAttribute("bd", board); // 객체받은걸 request에 집어 넣는것
			...
			//포워딩 = view한테 값을 내려주는것
			request.setAttribute("board", board); // 객체받은걸 request에 집어 넣는것
